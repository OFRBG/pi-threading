...
export function registerCommands(pi: ExtensionAPI, store: ThreadStore, inbox: Inbox) {
  ...
  pi.registerCommand("/thread-emit", {
    ...
    async handler(args, ctx) {
      ...
      ctx.ui.notify(
        `Event "${eventId}" fired. ${n} subscriber(s) notified.`,
        "info",
      );
    },
  });
  ...
  pi.registerCommand("/thread-send", {
    description:
      "Send a message to another thread: /thread-send <to> <type> <body...>",
    async handler(args, ctx) {
      ...
      if (!to || !type || !body || !validTypes.includes(type as MessageType)) {
        ctx.ui.notify(
          `Usage: /thread-send <to> <${validTypes.join("|")}> <body...>`,
          "warning",
        );
        return;
      }
      ...
      ctx.ui.notify(
        `${type} sent to ${to}. requestId=${requestId} (${delivered}).`,
        "info",
      );
    },
  });
