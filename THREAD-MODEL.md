...
**Channel-confusion nudges**: as a thread's context grows, it can drift into replying in plain text — which only reaches the human — instead of calling `thread_send`, even while holding an unresolved obligation. Two passive, once-per-silence reminders catch this at `turn_end`, neither of which ever forces a turn (a forced turn just goads the model into acting for its own sake). The narrower one fires while locked **In Sync**: a pure-text turn with nothing sent to the sync partner gets a reminder naming that partner. The general one fires for any **owed reply** (a received Brief/Question/Blocker) left unresolved through a pure-text turn — the reminder is built entirely from `store.owed`, so it can only ever name the real thread and requestId actually owed, never a guess. Both gate on a boolean so a long silent stretch queues exactly one reminder, not one per turn; the owed-reply guard's gate additionally re-arms once per run (at `agent_end`) so a persistently silent thread still gets one fresh, escalating nudge per run rather than exactly one for its entire life. The companion half of the owed-reply guard lives on the send side: `thread_send`'s Answer/Result path soft-warns (never hard-fails, since a Sync's closing Answer has no matching `OwedReply` to check against) when a reply's `requestId`/target don't actually match a real owed record — without it, a misdirected reply would still silently clear the ledger as if it had landed correctly.
...
Also fixed later: received Brief/Question/Blocker are now durable **owed replies** (surfaced in `thread_status` with the exact requestId to echo, cleared when the matching Result/Answer is sent) — a revived thread no longer has to guess correlation ids. Sends of locking types (Question/Blocker/Sync) to a thread id that has never run are refused instead of locking the sender forever on a typo; queueing types still dead-drop to future thread ids, with a warning in the tool result. Fired scheduled wakes are pruned from state instead of accumulating as `(fired)` entries. Correlation/barrier/wake ids carry a per-process counter so two mints in the same millisecond can't collide. The Restate service's `fireWake` spawns the revived `pi` with `--thread-storage restate`/`--thread-storage-url` and the thread's original cwd (see README for the `RESTATE_INGRESS_URL`/`PI_THREAD_EXTENSION` service env vars) — previously it spawned a `pi` that booted against the wrong (local) backend in the wrong directory. Per-turn journaling is rate-limited (see Journal above). The journal fork now runs `--no-extensions`: with pi-threading installed via extension discovery, each fork used to load the extension, create a ghost `thread-<uuid>` in the workspace, and spawn a further journal fork of its own — an unbounded chain (reproduced live before the fix; explicit `--extension` launches were immune because forks don't inherit that flag). Delivery into the local session was later gated (see Injection gate above) after field reports of threads stuck in "Working..." with undelivered notes sitting in pi's queue: per-envelope `sendUserMessage` calls raced pi's idle preflight (dropping the loser) and could fire mid-compaction (starting a run that races the context rewrite).

**Fixed most recently**: the In Sync channel-confusion nudge only ever covered the sync-lock case — a thread silently holding an owed Brief/Question/Blocker reply while producing pure-text turns had no guard at all, and this got materially worse as a thread's context grew (the more common failure was a small/cheap worker model drifting into prose instead of calling `thread_send`, not a sync partner specifically). Generalized to any owed reply (see Channel-confusion nudges above), with a send-side companion check so a misdirected Answer/Result can't silently clear the ledger as if it had landed correctly. Designed across two rounds of subagent review (an architect proposal, then two independent critiques) before being implemented — the second review caught that the first draft's escalation logic was unreachable (the gate and the counter it was meant to unlock cleared together, so a thread could only ever be nudged once in its entire life) and two bugs in the send-side snippet (an undeclared variable, and a comparison against the raw `to` spec instead of the resolved target list, which would have spuriously warned on `to="*"`/role/comma-list sends).
